...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
  ...
  if (!chat.nsfw)
    throw `💝 kenn onyx 💝  does not support NSFW.\n\nTo turn it on, use: ${usedPrefix}on nsfw \n\n🤤🤤🤤🤤🤤🤤🤤🤤🤤`;
  ...
  if (!text)
    throw `💝 kenn onyx 💝 , What do you want to search on 💝 kenn onyx 💝?\n🕸️ Usage: ${usedPrefix + command} <search>\n\nEx:Mia කලීපා 🤣  or bza can use a link \nEx: .xnxx link *`;

  m.react(rwait);

  if (text.includes('http://') || text.includes('https://')) {
    if (!text.includes('xnxx.com'))
      return m.reply(`Invalid link. Only xnxx.com, Generated 💝 kenn onyx 💝`);
...
🕸️ kenn-onyx
    ...
    try {
      ...
      if (searchJson.data && searchJson.data.length > 0) {
        ...
        m.reply(` No results found for the 💝 kenn onyx 💝 search list.`);
      }
    } catch (e) {
